...
metab_info_batch1 <- read_excel(metab_file_b1, sheet = metabolites_info_sheet_name)
metab_info_batch2 <- read_excel(metab_file_b2, sheet = metabolites_info_sheet_name)
...
# Added IDs for all test regions after scaling up the study
exp_group_ids <- c(100001266, 1114, 1021, 192, 100000007, 100006370,
                   100009332, 100001208, 2054, 100004046, 1224, 100006264)
...
#                                                                             "LABID", "SOL_ID", "BATCH")]
exp_group_names <- c("Nacetylarginine_std", "aminoisobutyrate_std",
                     "oxoproline_std", "Nacetylputrescine_std", "carnitine_std",
                     "betahydroxy5cholestenoate_std", "arachidonoylcholine_std",
                     "methylimidazoleacetate_std", "ethylmalonate_std",
                     "Nacetylcarnosine_std", "propyl4hydroxybenzoatesulfate_std")

metab_vals_batch1 <- metab_vals_batch1 |>
  left_join(metab_vals_batch1_raw[,c("LAB_ID", exp_group_names
                                     )],
            by = c("LABID" = "LAB_ID"))
...
metab_vals_batch2 <- merge(metab_vals_batch2, 
                           sample_info_batch2[,c("PARENT_SAMPLE_NAME", "SOL_ID")], 
                           by = c("PARENT_SAMPLE_NAME", "SOL_ID"))
...
metab_vals_batch1 <- metab_vals_batch1 |> filter(BATCH == "B01") # 3977 
...
metab_vals_batch1_metab <- metab_vals_batch1 |>
  mutate(Nacetylarginine_std = if_else(is.na(Nacetylarginine_std), 
                                       min(Nacetylarginine_std, na.rm= TRUE),
                                       Nacetylarginine_std)) |>
  mutate(ranks = rank(Nacetylarginine_std, ties.method = "random"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE)),
         x100001266 = (int_norm - min(int_norm, na.rm = TRUE)) / 
           (max(int_norm, na.rm = TRUE) - min(int_norm, na.rm = TRUE))) |>
  mutate(x100001266 = x100001266 - median(x100001266) + 1) |>
  select(-c(ranks, int_norm)) |>
  mutate(aminoisobutyrate_std = if_else(is.na(aminoisobutyrate_std), 
                                        min(aminoisobutyrate_std, na.rm= TRUE),
                                        aminoisobutyrate_std)) |>
  mutate(ranks = rank(aminoisobutyrate_std, ties.method = "random"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE)),
         x1114 = (int_norm - min(int_norm, na.rm = TRUE)) / 
           (max(int_norm, na.rm = TRUE) - min(int_norm, na.rm = TRUE))) |>
  mutate(x1114 = x1114 - median(x1114) + 1) |>
  select(-c(ranks, int_norm)) |>
  ...
  select(-c(ranks, int_norm)) |>
  mutate(Nacetylputrescine_std = if_else(is.na(Nacetylputrescine_std), 
                                  min(Nacetylputrescine_std, na.rm= TRUE),
                                  Nacetylputrescine_std)) |>
  mutate(ranks = rank(Nacetylputrescine_std, ties.method = "random"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE)),
         x192 = (int_norm - min(int_norm, na.rm = TRUE)) / 
           (max(int_norm, na.rm = TRUE) - min(int_norm, na.rm = TRUE))) |>
  mutate(x192 = x192 - median(x192) + 1) |>
  select(-c(ranks, int_norm)) |>
  ...
  select(-c(ranks, int_norm)) |>
  mutate(betahydroxy5cholestenoate_std = if_else(is.na(betahydroxy5cholestenoate_std), 
                                 min(betahydroxy5cholestenoate_std, na.rm= TRUE),
                                 betahydroxy5cholestenoate_std)) |>  
  mutate(ranks = rank(betahydroxy5cholestenoate_std, ties.method = "random"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE)),
         x100006370 = (int_norm - min(int_norm, na.rm = TRUE)) / 
           (max(int_norm, na.rm = TRUE) - min(int_norm, na.rm = TRUE))) |>
  mutate(x100006370 = x100006370 - median(x100006370) + 1) |> 
  select(-c(ranks, int_norm)) |>
  mutate(arachidonoylcholine_std = if_else(is.na(arachidonoylcholine_std), 
                                                 min(arachidonoylcholine_std, na.rm= TRUE),
                                                 arachidonoylcholine_std)) |>  
  mutate(ranks = rank(arachidonoylcholine_std, ties.method = "random"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE)),
         x100009332 = (int_norm - min(int_norm, na.rm = TRUE)) / 
           (max(int_norm, na.rm = TRUE) - min(int_norm, na.rm = TRUE))) |>
  mutate(x100009332 = x100009332 - median(x100009332) + 1) |> 
  select(-c(ranks, int_norm)) |>
  mutate(methylimidazoleacetate_std = if_else(is.na(methylimidazoleacetate_std), 
                                           min(methylimidazoleacetate_std, na.rm= TRUE),
                                           methylimidazoleacetate_std)) |>  
  mutate(ranks = rank(methylimidazoleacetate_std, ties.method = "random"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE)),
         x100001208 = (int_norm - min(int_norm, na.rm = TRUE)) / 
           (max(int_norm, na.rm = TRUE) - min(int_norm, na.rm = TRUE))) |>
  mutate(x100001208 = x100001208 - median(x100001208) + 1) |> 
  select(-c(ranks, int_norm)) |>
  mutate(ethylmalonate_std = if_else(is.na(ethylmalonate_std), 
                                              min(ethylmalonate_std, na.rm= TRUE),
                                     ethylmalonate_std)) |>  
  mutate(ranks = rank(ethylmalonate_std, ties.method = "random"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE)),
         x2054 = (int_norm - min(int_norm, na.rm = TRUE)) / 
           (max(int_norm, na.rm = TRUE) - min(int_norm, na.rm = TRUE))) |>
  mutate(x2054 = x2054 - median(x2054) + 1) |> 
  select(-c(ranks, int_norm)) |>
  ...
  mutate(ranks = rank(propyl4hydroxybenzoatesulfate_std, ties.method = "random"),
         int_norm = qnorm((ranks - 0.5) / max(ranks, na.rm = TRUE)),
         x100006264 = (int_norm - min(int_norm, na.rm = TRUE)) / 
           (max(int_norm, na.rm = TRUE) - min(int_norm, na.rm = TRUE))) |>
  mutate(x100006264 = x100006264 - median(x100006264) + 1) |> 
  select(-c(ranks, int_norm)) 
  

length(intersect(metab_vals_batch1$SOL_ID, metab_vals_batch2$SOL_ID)) #0

metab_selected_batch1 <- metab_vals_batch1_metab[,colnames(metab_vals_batch1_metab) %in% c("SOL_ID",
...
saveRDS(covariate_b1_all_metab, "../Data/covariate_b1_all_metab_scaled_up.RDS")

metab_selected_batch2 <- metab_vals_batch2[,colnames(metab_vals_batch2) %in% c("SOL_ID", exp_group_ids)] |>
  rename(ID = SOL_ID) |>
...
cols_to_impute <- colnames(covariate_b2_all_metab)[12:ncol(covariate_b2_all_metab)]
...
saveRDS(covariate_b2_all_metab, "../Data/covariate_b2_all_metab_scaled_up.RDS")
