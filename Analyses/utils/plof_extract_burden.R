plof_cond_extract_burden <- function(chr,gene_name,genofile,obj_nullmodel,genes,known_loci,
  ...
  }else
  {
    ...
    if(n_pheno == 1)
    {
      # print(paste0("dim Geno: ", dim(Geno)))
      # print(paste0("dim Geno adjusted: ", dim(Geno_adjusted)))
      rownames(Geno) <- phenotype.id.merge$phenotype.id
      rownames(Geno_adjusted) <- phenotype.id.merge$phenotype.id
      try(burden <- STAAR_cond_burden_score_only(Geno,Geno_adjusted,obj_nullmodel,Anno.Int.PHRED.sub,rare_maf_cutoff=rare_maf_cutoff,rv_num_cutoff=rv_num_cutoff,method_cond=method_cond))
    }
  }
