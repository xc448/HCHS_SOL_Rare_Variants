enhancer_CAGE_cond_extract_burden <- function(chr,gene_name,genofile,obj_nullmodel,known_loci,
  ...
  rOCRsAnno <- seqGetData(genofile, paste0(Annotation_dir,Annotation_name_catalog$dir[which(Annotation_name_catalog$name=="CAGE")]))
  rOCRs <- rOCRsAnno!=""
  ...
  }else
  {
    ...
    if(n_pheno == 1)
    {
      ...
      burden <- STAAR_cond_burden_score_only(Geno,Geno_adjusted,obj_nullmodel,Anno.Int.PHRED.sub,rare_maf_cutoff=rare_maf_cutoff,rv_num_cutoff=rv_num_cutoff,rv_num_cutoff_max=rv_num_cutoff_max,method_cond=method_cond)
    }
  }
