...
#' @param chr chromosome of interest, since the assciation results is not necessarily genome-wide 
...
#' and unconditional p-values of set-based tests SKAT ("SKAT(1,25)"), Burden ("Burden(1,25)"), ACAT-V ("ACAT-V(1,25)") and STAAR-O ("STAAR-O")
#' or unconditional p-values of set-based tests Burden ("Burden(1,25)") and STAAR-B ("STAAR-B") for imbalanced case-control setting.
...
#' including gene name ("Gene name"), chromosome ("chr"), coding functional category ("Category"), number of variants  ("#SNV"),
#' and conditional p-values of set-based tests SKAT ("SKAT(1,25)"), Burden ("Burden(1,25)"), ACAT-V ("ACAT-V(1,25)") and STAAR-O ("STAAR-O")
#' or conditional p-values of set-based tests Burden ("Burden(1,25)") and STAAR-B ("STAAR-B") for imbalanced case-control setting.
...
Gene_Centric_Coding_Results_Summary <- function(agds_dir,gene_centric_coding_jobs_num,input_path,output_path,gene_centric_results_name,
  ...
  {
    #print(kk)
    results_coding <- get(load(paste0(input_path,gene_centric_results_name,".Rdata")))
  ...
  if(use_SPA)
  {
    ...
    saveRDS(plof_sig, paste0(output_path,"plof_sig.RDS"))
    
    # missense
    missense_sig <- results_missense_genome[results_missense_genome[,"STAAR-B"]<alpha,,drop=FALSE]
    saveRDS(missense_sig, paste0(output_path,"missense_sig.RDS"))
    
    # synonymous
    synonymous_sig <- results_synonymous_genome[results_synonymous_genome[,"STAAR-B"]<alpha,,drop=FALSE]
    saveRDS(synonymous_sig, paste0(output_path,"synonymous_sig.RDS"))
    
    # plof_ds
    plof_ds_sig <- results_plof_ds_genome[results_plof_ds_genome[,"STAAR-B"]<alpha,,drop=FALSE]
    saveRDS(plof_ds_sig, paste0(output_path,"plof_ds_sig.RDS"))
    
    # disruptive_missense
    disruptive_missense_sig <- results_disruptive_missense_genome[results_disruptive_missense_genome[,"STAAR-B"]<alpha,,drop=FALSE]
    saveRDS(disruptive_missense_sig, paste0(output_path,"disruptive_missense_sig.RDS"))
    
    
    # coding results
    coding_sig <- rbind(plof_sig[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")],
                        missense_sig[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")])
    coding_sig <- rbind(coding_sig,synonymous_sig[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")])
    coding_sig <- rbind(coding_sig,plof_ds_sig[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")])
    coding_sig <- rbind(coding_sig,disruptive_missense_sig[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")])
    
    saveRDS(coding_sig, paste0(output_path,"coding_sig.RDS"))
  }else{
    # plof
    if(!is.null(results_plof_genome) & is.null(dim(results_plof_genome))){
      colnames_plof <- names(results_plof_genome)
      results_plof_genome <- matrix(unlist(results_plof_genome), nrow = 1)
    colnames(results_plof_genome) <- colnames_plof
    results_plof_genome[,"STAAR-O"] <- is.numeric(results_plof_genome[,"STAAR-O"])
    results_plof_genome[,"STAAR-B(1,1)"] <- is.numeric(results_plof_genome[,"STAAR-B(1,1)"])
    results_plof_genome[,"STAAR-B(1,25)"] <- is.numeric(results_plof_genome[,"STAAR-B(1,25)"])
    }
    plof_sig <- results_plof_genome[(results_plof_genome[,"STAAR-O"]<alpha |
                                       results_plof_genome[,"STAAR-B(1,1)"]<alpha |
                                       results_plof_genome[,"STAAR-B(1,25)"]<alpha),,drop=FALSE]
    saveRDS(plof_sig, paste0(output_path,"plof_sig.RDS"))
    
    # missense
    if(!is.null(results_missense_genome) & is.null(dim(results_missense_genome))){
      colnames_missense <- names(results_missense_genome)
      results_missense_genome <- matrix(unlist(results_missense_genome), nrow = 1)
      colnames(results_missense_genome) <- colnames_missense
      results_missense_genome[,"STAAR-O"] <- is.numeric(results_missense_genome[,"STAAR-O"])
      results_missense_genome[,"STAAR-B(1,1)"] <- is.numeric(results_missense_genome[,"STAAR-B(1,1)"])
      results_missense_genome[,"STAAR-B(1,25)"] <- is.numeric(results_missense_genome[,"STAAR-B(1,25)"])
    }
    missense_sig <- results_missense_genome[(results_missense_genome[,"STAAR-O"]<alpha |
                                               results_missense_genome[,"STAAR-B(1,1)"]<alpha |
                                               results_missense_genome[,"STAAR-B(1,25)"]<alpha),,drop=FALSE]
    saveRDS(missense_sig, paste0(output_path,"missense_sig.RDS"))
    
    # synonymous
    if(!is.null(results_synonymous_genome) & is.null(dim(results_synonymous_genome))){colnames_synonymous <- names(results_synonymous_genome)
    results_synonymous_genome <- matrix(unlist(results_synonymous_genome), nrow = 1)
    colnames(results_synonymous_genome) <- colnames_synonymous
    results_synonymous_genome[,"STAAR-O"] <- is.numeric(results_synonymous_genome[,"STAAR-O"])
    results_synonymous_genome[,"STAAR-B(1,1)"] <- is.numeric(results_synonymous_genome[,"STAAR-B(1,1)"])
    results_synonymous_genome[,"STAAR-B(1,25)"] <- is.numeric(results_synonymous_genome[,"STAAR-B(1,25)"])
    }
    synonymous_sig <- results_synonymous_genome[(results_synonymous_genome[,"STAAR-O"]<alpha |
                                                   results_synonymous_genome[,"STAAR-B(1,1)"]<alpha |
                                                   results_synonymous_genome[,"STAAR-B(1,25)"]<alpha),,drop=FALSE]
    saveRDS(synonymous_sig, paste0(output_path,"synonymous_sig.RDS"))
    
    # plof_ds
    if(!is.null(results_plof_ds_genome) & is.null(dim(results_plof_ds_genome))){
    colnames_plof_ds <- names(results_plof_ds_genome)
    results_plof_ds_genome <- matrix(unlist(results_plof_ds_genome), nrow = 1)
    colnames(results_plof_ds_genome) <- colnames_plof_ds
    print(results_plof_ds_genome[,"STAAR-O"])
    results_plof_ds_genome[,"STAAR-O"] <- is.numeric(results_plof_ds_genome[,"STAAR-O"])
    results_plof_ds_genome[,"STAAR-B(1,1)"] <- is.numeric(results_plof_ds_genome[,"STAAR-B(1,1)"])
    results_plof_ds_genome[,"STAAR-B(1,25)"] <- is.numeric(results_plof_ds_genome[,"STAAR-B(1,25)"])
    }
    plof_ds_sig <- results_plof_ds_genome[(results_plof_ds_genome[,"STAAR-O"]<alpha |
                                             results_plof_ds_genome[,"STAAR-B(1,1)"]<alpha |
                                             results_plof_ds_genome[,"STAAR-B(1,25)"]<alpha),,drop=FALSE]
    saveRDS(plof_ds_sig, paste0(output_path,"plof_ds_sig.RDS"))
    
    # disruptive_missense
    if(!is.null(results_disruptive_missense_genome) & is.null(dim(results_disruptive_missense_genome))){
    colnames_disrupt <- names(results_disruptive_missense_genome)
    results_disruptive_missense_genome <- matrix(unlist(results_disruptive_missense_genome), nrow = 1)
    colnames(results_disruptive_missense_genome) <- colnames_disrupt
    results_disruptive_missense_genome[,"STAAR-O"] <- is.numeric(results_disruptive_missense_genome[,"STAAR-O"])
    results_disruptive_missense_genome[,"STAAR-B(1,1)"] <- is.numeric(results_disruptive_missense_genome[,"STAAR-B(1,1)"])
    results_disruptive_missense_genome[,"STAAR-B(1,25)"] <- is.numeric(results_disruptive_missense_genome[,"STAAR-B(1,25)"])
    }
    disruptive_missense_sig <- results_disruptive_missense_genome[(results_disruptive_missense_genome[,"STAAR-O"]<alpha |
                                                                     results_disruptive_missense_genome[,"STAAR-B(1,1)"]<alpha |
                                                                     results_disruptive_missense_genome[,"STAAR-B(1,25)"]<alpha),,drop=FALSE]
    saveRDS(disruptive_missense_sig,paste0(output_path,"disruptive_missense_sig.RDS"))
    
    # coding results
    print(plof_sig)
    coding_sig <- rbind(plof_sig[,c("Gene name","Chr","Category","#SNV","cMAC","STAAR-S(1,25)",	"STAAR-S(1,1)",	"STAAR-B(1,25)",
                                    "STAAR-B(1,1)","STAAR-A(1,25)","STAAR-A(1,1)","ACAT-O",	"STAAR-O")],
                        missense_sig[,c("Gene name","Chr","Category","#SNV","cMAC","STAAR-S(1,25)",	"STAAR-S(1,1)",	"STAAR-B(1,25)",
                                        "STAAR-B(1,1)","STAAR-A(1,25)","STAAR-A(1,1)","ACAT-O",	"STAAR-O")])
    coding_sig <- rbind(coding_sig,synonymous_sig[,c("Gene name","Chr","Category","#SNV","cMAC","STAAR-S(1,25)",	"STAAR-S(1,1)",	"STAAR-B(1,25)",
                                                     "STAAR-B(1,1)","STAAR-A(1,25)","STAAR-A(1,1)","ACAT-O",	"STAAR-O")])
    coding_sig <- rbind(coding_sig,plof_ds_sig[,c("Gene name","Chr","Category","#SNV","cMAC","STAAR-S(1,25)",	"STAAR-S(1,1)",	"STAAR-B(1,25)",
                                                  "STAAR-B(1,1)","STAAR-A(1,25)","STAAR-A(1,1)","ACAT-O",	"STAAR-O")])
    coding_sig <- rbind(coding_sig,disruptive_missense_sig[,c("Gene name","Chr","Category","#SNV","cMAC","STAAR-S(1,25)",	"STAAR-S(1,1)",	"STAAR-B(1,25)",
                                                              "STAAR-B(1,1)","STAAR-A(1,25)","STAAR-A(1,1)","ACAT-O",	"STAAR-O")])
  ...
  if(length(known_loci)!=0)
  {
    if(!use_SPA)
    {
      ...
      saveRDS(plof_sig_cond, paste0(output_path,"plof_sig_cond.RDS"))
      ...
      saveRDS(missense_sig_cond, paste0(output_path,"missense_sig_cond.RDS"))
      ...
      saveRDS(synonymous_sig_cond, paste0(output_path,"synonymous_sig_cond.RDS"))
      ...
      saveRDS(plof_ds_sig_cond, paste0(output_path,"plof_ds_sig_cond.RDS"))
      
      ### disruptive_missense
      disruptive_missense_sig_cond <- c()
      if(length(disruptive_missense_sig)!=0)
      {
        print(disruptive_missense_sig)
      ...
      saveRDS(disruptive_missense_sig_cond,paste0(output_path,"disruptive_missense_sig_cond.RDS"))
      
      ## coding cond
      coding_sig_cond <- rbind(plof_sig_cond[,c("Gene name","Chr","Category","#SNV","SKAT(1,25)","Burden(1,25)","ACAT-V(1,25)","STAAR-O")],
                               missense_sig_cond[,c("Gene name","Chr","Category","#SNV","SKAT(1,25)","Burden(1,25)","ACAT-V(1,25)","STAAR-O")])
      coding_sig_cond <- rbind(coding_sig_cond,synonymous_sig_cond[,c("Gene name","Chr","Category","#SNV","SKAT(1,25)","Burden(1,25)","ACAT-V(1,25)","STAAR-O")])
      coding_sig_cond <- rbind(coding_sig_cond,plof_ds_sig_cond[,c("Gene name","Chr","Category","#SNV","SKAT(1,25)","Burden(1,25)","ACAT-V(1,25)","STAAR-O")])
      coding_sig_cond <- rbind(coding_sig_cond,disruptive_missense_sig_cond[,c("Gene name","Chr","Category","#SNV","SKAT(1,25)","Burden(1,25)","ACAT-V(1,25)","STAAR-O")])
      
      saveRDS(coding_sig_cond, paste0(output_path,"coding_sig_cond.RDS"))
    }else
    {
      ### synonymous
      synonymous_sig_cond <- c()
      ...
      # coding results
      coding_sig_cond <- rbind(plof_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")],
                               missense_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")])
      coding_sig_cond <- rbind(coding_sig_cond,synonymous_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")])
      coding_sig_cond <- rbind(coding_sig_cond,plof_ds_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")])
      coding_sig_cond <- rbind(coding_sig_cond,disruptive_missense_sig_cond[,c("Gene name","Chr","Category","#SNV","cMAC","Burden(1,25)","STAAR-B")])
  ...
  if(manhattan_plot)
  {
    ...
    genes_info_chr <- genes_info |> filter(chromosome_name == chr)
    genes_info_manhattan <- dplyr::left_join(genes_info_chr,results_m,by=c("chromosome_name"="Chr","hgnc_symbol"="Gene.name"))
    
    genes_info_manhattan[is.na(genes_info_manhattan)] <- 1
    colnames(genes_info_manhattan)[dim(genes_info_manhattan)[2]] <- "plof"
    
    ### plof_ds
    col.names <- names(results_plof_ds_genome)
    results_plof_ds_genome <- matrix(unlist(results_plof_ds_genome), nrow = 1)
    colnames(results_plof_ds_genome) <- col.names
    
    results_STAAR <- as.data.frame(results_plof_ds_genome[,c(1,2,dim(results_plof_ds_genome)[2])])
