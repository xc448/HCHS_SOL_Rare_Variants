...
#' @return \code{noncoding_sig.RDS}: a matrix that summarized the unconditional significant region detected by STAAR-O or STAAR-B in imbalanced case-control setting (STAAR-O/-B pvalue smaller than the threshold alpha),
...
#' @return \code{noncoding_sig_cond.RDS}: a matrix that summarized the conditional analysis results of the unconditional significant region detected by STAAR-O or STAAR-B in imbalanced case-control setting (available if known_loci is not a NULL),
#' including gene name ("Gene name"), chromosome ("chr"), coding functional category ("Category"), number of variants  ("#SNV"),
...
#' @return \code{UTR_sig.RDS}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant UTR masks.
#' @return \code{UTR_sig_cond.RDS}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant UTR masks (available if known_loci is not a NULL).
...
#' @return \code{upstream_sig.RDS}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant upstream masks.
#' @return \code{upstream_sig_cond.RDS}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant upstream masks (available if known_loci is not a NULL).
...
#' @return \code{downstream_sig.RDS}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant downstream masks.
#' @return \code{downstream_sig_cond.RDS}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant downstream masks (available if known_loci is not a NULL).
...
#' @return \code{promoter_CAGE_sig.RDS}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant promoter_CAGE masks.
#' @return \code{promoter_CAGE_sig_cond.RDS}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant promoter_CAGE masks (available if known_loci is not a NULL).
...
#' @return \code{promoter_DHS_sig.RDS}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant promoter_DHS masks.
#' @return \code{promoter_DHS_sig_cond.RDS}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant promoter_DHS masks (available if known_loci is not a NULL).
...
#' @return \code{enhancer_CAGE_sig.RDS}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant enhancer_CAGE masks.
#' @return \code{enhancer_CAGE_sig_cond.RDS}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant enhancer_CAGE masks (available if known_loci is not a NULL).
...
#' @return \code{enhancer_DHS_sig.RDS}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant enhancer_DHS masks.
#' @return \code{enhancer_DHS_sig_cond.RDS}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant enhancer_DHS masks (available if known_loci is not a NULL).
...
#' @return \code{ncRNA_sig.RDS}: a matrix contains the unconditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant ncRNA masks.
#' @return \code{ncRNA_sig_cond.RDS}: a matrix contains the conditional STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) of the unconditional significant ncRNA masks (available if known_loci is not a NULL).
...
Gene_Centric_Noncoding_Results_Summary <- function(agds_dir,gene_centric_noncoding_jobs_num,input_path,output_path,gene_centric_results_name,
  ...
  {
    print(kk)
    results_noncoding <- get(load(paste0(input_path,gene_centric_results_name, ".Rdata")))
  ...
  # {
  #   print(kk)
  #   results_ncRNA <- get(load(paste0(ncRNA_input_path,ncRNA_results_name, ".Rdata")))
  ...
  {
    ### UTR
    UTR_sig <- results_UTR_genome[(results_UTR_genome[,"STAAR-B(1,1)"]<alpha |
                                    results_UTR_genome[,"STAAR-B(1,25)"]<alpha |  
                                  results_UTR_genome[,"STAAR-O"]<alpha),,drop=FALSE]
    saveRDS(UTR_sig, paste0(output_path,"UTR_sig.RDS"))
    
    noncoding_sig <- c()
    noncoding_sig <- rbind(noncoding_sig,UTR_sig)
    
    ### upstream
    if(length(dim(results_upstream_genome)) > 1){
        upstream_sig <- results_upstream_genome[(results_upstream_genome[,"STAAR-B(1,1)"]<alpha |
                                                   results_upstream_genome[,"STAAR-B(1,25)"]<alpha |  
                                                   results_upstream_genome[,"STAAR-O"]<alpha),,drop=FALSE]
        ...
        upstream_sig <- results_upstream_genome[(results_upstream_genome["STAAR-B(1,1)"]<alpha |
                                                   results_upstream_genome["STAAR-B(1,25)"]<alpha |  
                                                   results_upstream_genome["STAAR-O"]<alpha),drop=FALSE]
        print(results_upstream_genome["STAAR-B(1,1)"])
      }
      saveRDS(upstream_sig, paste0(output_path,"upstream_sig.RDS"))
      noncoding_sig <- rbind(noncoding_sig,upstream_sig)
    
    
    ### downstream
    if(length(dim(results_downstream_genome)) > 1){
      downstream_sig <- results_downstream_genome[(results_downstream_genome[,"STAAR-B(1,1)"]<alpha |
                                                   results_downstream_genome[,"STAAR-B(1,25)"]<alpha |  
                                                   results_downstream_genome[,"STAAR-O"]<alpha),,drop=FALSE]
    }else{
      downstream_sig <- results_downstream_genome[(results_downstream_genome["STAAR-B(1,1)"]<alpha |
                                                     results_downstream_genome["STAAR-B(1,25)"]<alpha |  
                                                     results_downstream_genome["STAAR-O"]<alpha),drop=FALSE] 
      }
    saveRDS(downstream_sig, paste0(output_path,"downstream_sig.RDS"))
    
    # print(downstream_sig)
    # downstream_sig <- t(matrix(unlist(downstream_sig), nrow = 91, byrow = TRUE))
    # colnames(downstream_sig) <- colnames(noncoding_sig)
    # noncoding_sig <- rbind(noncoding_sig, downstream_sig)
  
    
    ### promoter_CAGE
    if(length(dim(results_promoter_CAGE_genome)) > 1){
    promoter_CAGE_sig <- results_promoter_CAGE_genome[(results_promoter_CAGE_genome[,"STAAR-B(1,1)"]<alpha |
                                                         results_promoter_CAGE_genome[,"STAAR-B(1,25)"]<alpha |  
                                                         results_promoter_CAGE_genome[,"STAAR-O"]<alpha),,drop=FALSE]
    }else{
    promoter_CAGE_sig <- results_promoter_CAGE_genome[(results_promoter_CAGE_genome["STAAR-B(1,1)"]<alpha |
                                                         results_promoter_CAGE_genome["STAAR-B(1,25)"]<alpha |  
                                                         results_promoter_CAGE_genome["STAAR-O"]<alpha),drop=FALSE]
    }
    saveRDS(promoter_CAGE_sig,paste0(output_path,"promoter_CAGE_sig.RDS"))
    
    noncoding_sig <- rbind(noncoding_sig,promoter_CAGE_sig)
    
    ### promoter_DHS
    if(length(dim(results_promoter_DHS_genome)) > 1){
    promoter_DHS_sig <- results_promoter_DHS_genome[(results_promoter_DHS_genome[,"STAAR-B(1,1)"]<alpha |
                                                       results_promoter_DHS_genome[,"STAAR-B(1,25)"]<alpha |  
                                                       results_promoter_DHS_genome[,"STAAR-O"]<alpha),,drop=FALSE]
    }else{
    promoter_DHS_sig <- results_promoter_DHS_genome[(results_promoter_DHS_genome["STAAR-B(1,1)"]<alpha |
                                                       results_promoter_DHS_genome["STAAR-B(1,25)"]<alpha |  
                                                       results_promoter_DHS_genome["STAAR-O"]<alpha),drop=FALSE]
    }
    saveRDS(promoter_DHS_sig, paste0(output_path,"promoter_DHS_sig.RDS"))
    
    noncoding_sig <- rbind(noncoding_sig,promoter_DHS_sig)
    
    ### enhancer_CAGE
    if(length(dim(results_enhancer_CAGE_genome)) > 1){
    enhancer_CAGE_sig <- results_enhancer_CAGE_genome[(results_enhancer_CAGE_genome[,"STAAR-B(1,1)"]<alpha |
                                                         results_enhancer_CAGE_genome[,"STAAR-B(1,25)"]<alpha |  
                                                         results_enhancer_CAGE_genome[,"STAAR-O"]<alpha),,drop=FALSE]
    }else{
    enhancer_CAGE_sig <- results_enhancer_CAGE_genome[(results_enhancer_CAGE_genome["STAAR-B(1,1)"]<alpha |
                                                         results_enhancer_CAGE_genome["STAAR-B(1,25)"]<alpha |  
                                                         results_enhancer_CAGE_genome["STAAR-O"]<alpha),drop=FALSE]
    ...
    saveRDS(enhancer_CAGE_sig,paste0(output_path,"enhancer_CAGE_sig.RDS"))
    
    noncoding_sig <- rbind(noncoding_sig,enhancer_CAGE_sig)
    
    ### enhancer_DHS
    if(length(dim(results_enhancer_DHS_genome)) > 1){
    enhancer_DHS_sig <- results_enhancer_DHS_genome[(results_enhancer_DHS_genome[,"STAAR-B(1,1)"]<alpha |
                                                       results_enhancer_DHS_genome[,"STAAR-B(1,25)"]<alpha |  
                                                       results_enhancer_DHS_genome[,"STAAR-O"]<alpha),,drop=FALSE]
    }else{
    enhancer_DHS_sig <- results_enhancer_DHS_genome[(results_enhancer_DHS_genome["STAAR-B(1,1)"]<alpha |
                                                       results_enhancer_DHS_genome["STAAR-B(1,25)"]<alpha |  
                                                       results_enhancer_DHS_genome["STAAR-O"]<alpha),drop=FALSE]  
    }
    saveRDS(enhancer_DHS_sig,paste0(output_path,"enhancer_DHS_sig.RDS"))
    
    noncoding_sig <- rbind(noncoding_sig,enhancer_DHS_sig)
    
    ### ncRNA
    if(length(results_ncRNA_genome) > 0){
      if(is.null(dim(results_ncRNA_genome))){
        col.names <- names(results_ncRNA_genome)
        results_ncRNA_genome <- matrix(unlist(results_ncRNA_genome), nrow = length(results_ncRNA_genome)/91)
        colnames(results_ncRNA_genome) <- col.names
      }
      
      ncRNA_sig <- results_ncRNA_genome[(results_ncRNA_genome[,"Burden(1,1)"]<alpha_ncRNA |
                                         results_ncRNA_genome[,"STAAR-O"]<alpha_ncRNA),,drop=FALSE]
      saveRDS(ncRNA_sig, paste0(ncRNA_output_path,"ncRNA_sig.RDS"))
      noncoding_sig <- rbind(noncoding_sig,ncRNA_sig)
      
    }
    
    
    saveRDS(noncoding_sig,paste0(output_path,"noncoding_sig.RDS"))
  ...
  if(length(known_loci)!=0)
  {
    if(!use_SPA)
    {
      ...
      saveRDS(UTR_sig_cond, paste0(output_path,"UTR_sig_cond.RDS"))
      ...
      saveRDS(upstream_sig_cond, paste0(output_path,"upstream_sig_cond.RDS"))
      ...
      saveRDS(downstream_sig_cond, paste0(output_path,"downstream_sig_cond.RDS"))
      ...
      saveRDS(promoter_CAGE_sig_cond, paste0(output_path,"promoter_CAGE_sig_cond.RDS"))
      ...
      saveRDS(promoter_DHS_sig_cond, paste0(output_path,"promoter_DHS_sig_cond.RDS"))
      ...
      saveRDS(enhancer_CAGE_sig_cond, paste0(output_path,"enhancer_CAGE_sig_cond.RDS"))
      ...
      saveRDS(enhancer_DHS_sig_cond, paste0(output_path,"enhancer_DHS_sig_cond.RDS"))
      ...
      saveRDS(ncRNA_sig_cond, paste0(ncRNA_output_path,"ncRNA_sig_cond.RDS"))
      noncoding_sig_cond <- rbind(noncoding_sig_cond,ncRNA_sig_cond)
      
      ## noncoding
      saveRDS(noncoding_sig_cond, paste0(output_path,"noncoding_sig_cond.RDS"))
    }else
    {
      ### UTR
      UTR_sig_cond <- c()
  ...
  if(length(known_loci)!=0)
  {return(noncoding_sig_cond)}else{
    return(noncoding_sig)
  }
