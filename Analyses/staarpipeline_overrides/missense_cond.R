missense_cond <- function(chr,gene_name,genofile,obj_nullmodel,genes,known_loci,
  ...
  }else
  {
    ...
    {
      Geno <- matrix(Geno)
      Geno_adjusted <- matrix(Geno_adjusted)
  ...
  }else
  {
    ...
    if(inherits(pvalues, "list"))
    {
      ...
      results <- rbind(results,results_temp)
      print(paste0("dim results is ", dim(results)))
    }
  ...
  if(!is.null(dim(results[1]))){
    print("HUH")
    if(!is.null(dim(results)) & dim(results)[1]==1)
    {
    ...
    if(!is.null(dim(results)) & dim(results)[1]==2)
    {
