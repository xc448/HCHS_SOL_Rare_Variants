...
Individual_Analysis_cond <- function(chr,individual_results,genofile,obj_nullmodel,known_loci=NULL,
  ...
  if(dim(known_loci_chr)[1]>=1)
  {
    ...
    # harmonizing to avoid multi-allelic notations 
    Chr_Info <- as.data.frame(Chr_Info)
    known_loci_chr <- known_loci_chr %>%
      rowwise() %>%
      mutate(
        in_B = any(
          Chr_Info$CHR == CHR &
            Chr_Info$POS == POS &
            # partial match of REF
            sapply(Chr_Info$REF, function(x) grepl(REF, x)) &
            # partial match of ALT
            sapply(Chr_Info$ALT, function(x) grepl(ALT, x))
        )
      ) %>%
      ungroup()
    
    known_loci_chr <- left_join(known_loci_chr[, 1:3],Chr_Info,by=c("CHR"="CHR","POS"="POS","REF"="REF"))
    test <- left_join(known_loci_chr[, 1:2],Chr_Info,by=c("CHR"="CHR","POS"="POS"))
    print(test)
    known_loci_chr <- known_loci_chr[!is.na(known_loci_chr$variant_id),]
    print(paste0("dim known loci: ", dim(known_loci_chr)))
    ...
    if(!geno_position_ascending)
    {
      ...
      known_loci_chr_adjusted <- dplyr::left_join(known_loci_chr,Chr_Info_adjusted,by=c("CHR"="CHR","POS"="POS","REF"="REF"))
  ...
  individual_results <- individual_results[!is.na(individual_results$variant_id),]
  ...
  #tot_ind_results <- rbind(known_loci_chr, individual_results)
